...
// Note: AdventureOutlineStageSchema is used by AdventureOutlineSchema, so it's defined first.
...
export const AdventureOutlineSchema: Schema = {
  ...
  properties: {
    ...
    stages: {
      ...
      items: AdventureOutlineStageSchema // Reference to the schema above
    }
  },
...
export const StorySegmentSchema: Schema = {
  ...
  properties: {
    ...
    itemFound: {
      // Spreading properties from another schema is not standard JSON Schema.
      // Instead, define it as an object and reference StorySegmentItemFoundSchema if it's complex,
      // or inline its properties. For AI schema hints, direct property definition is safer.
      type: Type.OBJECT,
      nullable: true,
      description:
      ...
      // required: ['name', 'description'] // Only required if itemFound is not null
    },
