...
export default function createAdventureApiRouter(ai: GoogleGenAI | undefined) {
  ...
  if (!ai) {
    router.use((req, res, next) => {
      res
        .status(503)
        .json({ error: 'AI Service is not available (API Key issue).' });
    });
    return router;
  }
  ...
  router.post(
    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        const { genre, persona } = req.body as {
          genre?: GameGenre;
          persona?: Persona;
        };
        ...
        if (
          typeof genre !== 'string' ||
          !GENRE_OPTIONS.includes(genre as GameGenre)
        ) {
          res
            .status(400)
            .json({ error: 'Missing or invalid required parameter: genre' });
          return;
        }
        if (
          typeof persona !== 'string' ||
          !PERSONA_OPTIONS.includes(persona as Persona)
        ) {
          res
            .status(400)
            .json({ error: 'Missing or invalid required parameter: persona' });
          return;
        }
        ...
        if (
          ...
          throw new JsonParseError(
            'Received incomplete adventure outline.',
  ...
  router.post(
    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        const { adventureOutline, persona, genre } = req.body as {
          adventureOutline?: AdventureOutline;
          persona?: Persona;
          genre?: GameGenre;
        };
        if (!adventureOutline || typeof adventureOutline !== 'object') {
          // Basic check for object
          res
            .status(400)
            .json({
              error: 'Missing or invalid required parameter: adventureOutline'
            });
          return;
        }
        if (
        ...
        const prompt = generateWorldDetailsPrompt(
          adventureOutline,
          persona,
          genre
        );
        ...
        if (
          ...
          throw new JsonParseError(
            'Received incomplete or malformed world details.',
  ...
  router.post(
    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        const { fullPrompt, isInitialScene } = req.body as {
          fullPrompt?: string;
          isInitialScene?: boolean;
        };
        if (typeof fullPrompt !== 'string') {
          // fullPrompt can be empty, but must be a string
          res
            .status(400)
            .json({
              error: 'Missing or invalid required parameter: fullPrompt'
            });
          return;
        }
        // isInitialScene is optional, defaults to false if undefined
        const currentIsInitialScene =
          isInitialScene === undefined ? false : isInitialScene;

        const augmentedPrompt = generateStorySegmentPrompt(
          fullPrompt,
          currentIsInitialScene
        );
        const result = await ai.models.generateContent({
          model: GENAI_MODEL_NAME,
        ...
        if (
          ...
          throw new JsonParseError(
            'Received incomplete story data.',
            JSON.stringify(storyData)
          );
        }
        if (
          storyData.isUserInputCommandOnly &&
          storyData.choices.length !== 0
        ) {
          throw new JsonParseError(
            'isUserInputCommandOnly is true but choices not empty.',
            JSON.stringify(storyData)
          );
        }
    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        if (
          ...
          !GENRE_OPTIONS.includes(selectedGenre as GameGenre) ||
          typeof selectedPersona !== 'string' ||
          !PERSONA_OPTIONS.includes(selectedPersona as Persona) ||
          inventory === undefined ||
          ...
          res
            ...
            .json({
              error:
                'Missing or invalid one or more required parameters for evaluate-action'
            });
          return;
        }
        ...
        const feasibilityData =
          parseJsonFromText<GeminiActionFeasibilityResponse>(result.text);
        if (
          typeof feasibilityData.isPossible !== 'boolean' ||
          typeof feasibilityData.reason !== 'string'
        ) {
          throw new JsonParseError(
            'Received incomplete action feasibility data.',
            JSON.stringify(feasibilityData)
          );
        }
    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        if (
          ...
          typeof currentStageIndex !== 'number' ||
          !feasibilityContext ||
          typeof feasibilityContext.wasImpossible !== 'boolean'
        ) {
          res
            .status(400)
            .json({
              error:
                'Missing or invalid one or more required parameters for custom-action-outcome'
            });
          return;
        }
        ...
        if (
          ...
          throw new JsonParseError(
            'Received incomplete story data for custom action.',
            JSON.stringify(storyData)
          );
        }
        let finalItemFound: InventoryItem | undefined = undefined;
  ...
  router.post(
    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        const { faultyJsonText, originalPromptContext } = req.body as {
          faultyJsonText?: string;
          originalPromptContext?: string;
        };
        if (
          typeof faultyJsonText !== 'string' ||
          typeof originalPromptContext !== 'string'
        ) {
          res
            .status(400)
            .json({
              error:
                'Missing or invalid required parameters: faultyJsonText, originalPromptContext'
            });
          return;
        }
        const fixPrompt = generateAttemptToFixJsonPrompt(
          faultyJsonText,
        ...
        if (
          ...
          throw new JsonParseError(
            'Received incomplete story data after fix attempt.',
            JSON.stringify(storyData)
          );
        }
        let finalItemFound: InventoryItem | undefined = undefined;
  ...
  router.post(
    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        if (
          typeof currentSceneDescription !== 'string' ||
          typeof adventureGenre !== 'string' ||
          !GENRE_OPTIONS.includes(adventureGenre as GameGenre) ||
          !adventureOutline ||
          !worldDetails ||
          ...
          res
            ...
            .json({
              error:
                'Missing one or more invalid required parameters for scene-examination'
            });
          return;
        }
        ...
        if (
          ...
          throw new JsonParseError(
            'Received incomplete examination data.',
    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        if (typeof promptBody !== 'string') {
          // promptBody can be empty, but must be a string
          res
            .status(400)
            .json({ error: 'Missing or invalid required parameter: prompt' });
          return;
        }
        let imageUrl = '';
        if (USE_IMAGEN) {
        ...
        if (!imageUrl) {
          console.warn(
            `Image generation for prompt "${promptBody}" resulted in an empty URL/string.`
          );
        }
