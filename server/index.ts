...
} from '@google/genai'; // Corrected type
...
app.post(
  ...
  async (
    req: express.Request<never, any, GenerateContentPayload>,
    res: express.Response
  ) => {
    if (!ai) {
...
app.post(
  '/api/generate-images',
  async (
    req: express.Request<never, any, GenerateImagesPayload>,
    res: express.Response
  ) => {
    if (!ai) {
      return res.status(503).json({
...
app.listen(port, () => {
  ...
  if (API_KEY && ai) {
    ...
    console.error(
      'Proxy: API_KEY is NOT LOADED or AI client failed to initialize. API calls will fail.'
    );
  }
});
