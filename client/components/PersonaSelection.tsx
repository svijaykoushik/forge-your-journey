...
const defaultPersonaDescriptions: Record<Persona, string> = {
  'Cautious Scholar':
    'Values knowledge and careful planning. Prefers to understand a situation fully before acting.',
  'Brave Warrior':
    'Faces challenges head-on with courage and strength. Quick to act and defend.',
  'Cunning Rogue':
    'Relies on wit, stealth, and trickery. Adept at finding unconventional solutions.',
  'Mysterious Wanderer':
    'An enigmatic figure with unclear motives, often an outsider observing and adapting.'
};
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-gray-900 text-gray-100">
      <div className="bg-gray-800 p-8 rounded-xl shadow-2xl w-full max-w-2xl text-center border border-gray-700">
        <p className="text-base text-indigo-300 mb-2">Genre: {selectedGenre}</p>{' '}
        {/* Increased font size from text-sm */}
        ...
        <p className="text-gray-400 mb-8 text-base sm:text-lg font-['Alegreya_Sans']">
          Your choice will subtly shape your adventure within the{' '}
          {selectedGenre} world.
        </p>{' '}
        {/* Increased font size */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {PERSONA_OPTIONS.map((persona) => {
            const genreSpecificDetails: GenrePersonaDetail | undefined =
              genrePersonaDetails[selectedGenre]?.[persona];
            const displayTitle = genreSpecificDetails?.title || persona;
            const displayDescription =
              genreSpecificDetails?.description ||
              defaultPersonaDescriptions[persona];
            ...
            return (
              ...
              >
                ...
                <p className="text-base text-purple-100 opacity-90 flex-grow font-['Alegreya_Sans']">
                  {displayDescription}
                </p>{' '}
                {/* Increased font size from text-sm */}
              </button>
      ...
      <footer className="mt-8 text-center text-sm text-gray-500">
        ...
        <p>
          &copy; {new Date().getFullYear()} Forge your Journey. All rights
          reserved (not really).
        </p>
      </footer>
    </div>
