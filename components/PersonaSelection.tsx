  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-gray-900 text-gray-100">
      <div className="bg-gray-800 p-8 rounded-xl shadow-2xl w-full max-w-2xl text-center border border-gray-700">
        ...
        <p className="text-gray-400 mb-8 text-base sm:text-lg">Your choice will subtly shape your adventure within the {selectedGenre} world.</p> {/* Increased font size */}
          ...
          {PERSONA_OPTIONS.map((persona) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-base text-purple-100 opacity-90 flex-grow">{displayDescription}</p> {/* Increased font size from text-sm */}
              </button>
            );
          })}
        </div>
       ...
       <footer className="mt-8 text-center text-sm text-gray-500"> {/* Increased from text-xs */}
        <p>&copy; {new Date().getFullYear()} Gemini Adventure RPG. All rights reserved (not really).</p>
      </footer>
    </div>
  );
};
