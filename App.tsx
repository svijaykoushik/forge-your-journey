...
const storyLoadingTexts = [
  ...
  "Considering your bold move...", "The world reacts to your decision..."
];
...
const customActionLoadingTexts = [
    "The threads of fate respond to your will...", "Evaluating your custom action...", "The world considers your input..."
];
...
const App: React.FC = () => {
  ...
  const [showImageQuotaNotification, setShowImageQuotaNotification] = useState(false);


  useEffect(() => {
    const imageEnvVar = process.env.IMAGE_GENERATION_ENABLED?.toLowerCase();
    const envVarDisabled = imageEnvVar === 'false' || imageEnvVar === 'disabled' || imageEnvVar === '0';

    const storedQuotaDisabled = localStorage.getItem(IMAGE_QUOTA_DISABLED_KEY) === 'true';
    setGameState(prev => ({ ...prev, imageGenerationPermanentlyDisabled: storedQuotaDisabled }));
      ...
      try {
        ...
        if (loadedState.selectedGenre &&
          ...
          setShowResumeModal(true);
           setGameState(prev => ({...prev, imageGenerationPermanentlyDisabled: loadedState.imageGenerationPermanentlyDisabled}));
           setImageGenerationFeatureEnabled(imageGenerationFeatureEnabled && !loadedState.imageGenerationPermanentlyDisabled); 
        } else {
          console.warn("Saved game state was missing essential fields. Discarding.");
  ...
  const handleGenreSelected = (genre: GameGenre) => {
    ...
    setGameState(prev => ({
      ...
      lastRetryInfo: null,
      error: null,
      journal: prev.journal.filter(entry => entry.type === 'genre_selected' || entry.type === 'system' && entry.content.includes("Image generation disabled")),
    }));
  ...
  const processSuccessfulSegment = useCallback(async (segmentData: StorySegment) => {
    ...
    const shouldAttemptImageLoad = imageGenerationFeatureEnabled && !gameState.imageGenerationPermanentlyDisabled && !!segmentData.imagePrompt && !(segmentData.isFailureScene || segmentData.isFinalScene);
    ...
    if (shouldAttemptImageLoad) {
      ...
      try {
        ...
        if (imgErr instanceof ImageGenerationQuotaError) {
            addJournalEntry('system', "Image generation quota exceeded. Visuals disabled for this session.");
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [addJournalEntry, gameState.inventory, imageGenerationFeatureEnabled, gameState.imageGenerationPermanentlyDisabled]);


  const loadStoryScene = useCallback(async (prompt: string, isRetryAttempt: boolean = false, forCustomAction: boolean = false) => {
    const shouldAttemptImageLoadInitial = imageGenerationFeatureEnabled && !gameState.imageGenerationPermanentlyDisabled;
    if (!isRetryAttempt && !gameState.isLoadingStory) {
       setCurrentLoadingText(getRandomLoadingText(forCustomAction ? customActionLoadingTexts : storyLoadingTexts));
       setGameState(prev => ({
        ...prev,
        isLoadingStory: true,
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
      const retryPrompt = gameState.lastRetryInfo?.customActionText ? gameState.lastRetryInfo.originalPrompt : prompt;
      ...
      if (err instanceof JsonParseError) {
        setGameState(prev => ({
          ...
          lastRetryInfo: { type: 'fix_json', originalPrompt: retryPrompt, faultyJsonText: err.rawText, customActionText: forCustomAction ? gameState.lastRetryInfo?.customActionText : undefined }
        }));
      } else {
        setGameState(prev => ({
          ...prev,
          ...
          lastRetryInfo: { type: 'resend_original', originalPrompt: retryPrompt, customActionText: forCustomAction ? gameState.lastRetryInfo?.customActionText : undefined }
        }));
      }
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [processSuccessfulSegment, gameState.lastRetryInfo, gameState.isLoadingStory, imageGenerationFeatureEnabled, gameState.imageGenerationPermanentlyDisabled, gameState.currentSegment]);

  useEffect(() => {
    if (
      gameState.adventureOutline && gameState.worldDetails &&
      gameState.selectedGenre && gameState.selectedPersona &&
      !gameState.currentSegment && !gameState.isLoadingOutline && !gameState.isLoadingWorld &&
      !gameState.isLoadingStory && !gameState.error && !showResumeModal &&
      !gameState.isGameEnded && !gameState.isGameFailed && !isSelectingGenre && !isSelectingPersona
    ) {
...
The sceneDescription should narrate the consequences (positive or negative) of the player's choice.`;
  ...
  const handleCustomActionSubmit = async (actionText: string) => {
    ...
    try {
      ...
      if (newSegment.sceneDescription.includes("The situation remains largely unchanged") || newSegment.sceneDescription.includes("decide against it")) {
      ...
      if (err instanceof JsonParseError) {
        setGameState(prev => ({
          ...prev,
          isLoadingStory: false, isLoadingImage: false,
          error: `Failed to parse outcome of custom action: ${errorMessage} Click Retry.`,
          lastRetryInfo: { type: 'fix_json', originalPrompt: contextForRetry, faultyJsonText: err.rawText, customActionText: actionText }
        }));
      } else {
        setGameState(prev => ({
          ...prev,
          ...
          lastRetryInfo: { type: 'resend_original', originalPrompt: contextForRetry, customActionText: actionText }
        }));
      }
    }
  };
  ...
  const handleResumeGame = () => {
    if (initialLoadedState) {
      const shouldLoadImageOnResume = imageGenerationFeatureEnabled &&
                                     ...
                                     !initialLoadedState.isGameFailed && !initialLoadedState.isGameEnded;
      setGameState(prev => ({
        ...prev,
      ...
      if (shouldLoadImageOnResume) {
        ...
        generateImage(initialLoadedState.currentSegment!.imagePrompt!)
          ...
          .catch(imgErr => {
            ...
            const imgErrorMessage = imgErr instanceof Error ? imgErr.message : 'Unknown image generation error.';
            if (imgErr instanceof ImageGenerationQuotaError) {
                addJournalEntry('system', "Image generation quota exceeded on resume. Visuals disabled.");
                setShowImageQuotaNotification(true);
                setGameState(prev => ({
                    ...prev,
                    imageGenerationPermanentlyDisabled: true,
  ...
  const handleRestartGame = (isFromErrorModal: boolean = false) => {
    ...
    if (stillQuotaDisabled && !isFromErrorModal){
        addJournalEntry('system', 'Note: Image generation remains disabled due to previous usage limits.');
        setShowImageQuotaNotification(true);
    } else {
  ...
  const handleRetry = async () => {
    ...
    const { type, originalPrompt, faultyJsonText, customActionText } = gameState.lastRetryInfo;
    ...
    if (type === 'fix_json' && faultyJsonText) {
      ...
      try {
        ...
        setGameState(prev => ({
          ...
          lastRetryInfo: { type: 'resend_original', originalPrompt: originalPrompt, customActionText: customActionText }
        }));
      }
  ...
  const handleReloadImageAttempt = async () => {
    if (!gameState.currentSegment || !gameState.currentSegment.imagePrompt || !imageGenerationFeatureEnabled || gameState.imageGenerationPermanentlyDisabled) {
        addJournalEntry('system', 'Image reload skipped: No prompt or image generation disabled.');
    ...
    try {
      const imageUrl = await generateImage(gameState.currentSegment.imagePrompt);
      setGameState(prev => ({
        ...prev,
        isLoadingImage: false,
        currentSegment: prev.currentSegment ? { ...prev.currentSegment, imageUrl } : null,
      ...
      if (imgErr instanceof ImageGenerationQuotaError) {
          addJournalEntry('system', "Image generation quota exceeded on retry. Visuals disabled.");
          setShowImageQuotaNotification(true);
          setGameState(prev => ({
              ...prev,
              imageGenerationPermanentlyDisabled: true,
          ...
          addJournalEntry('system', `Image reload failed again: ${imgErrorMessage}.`);
          setGameState(prev => ({
              ...prev,
              isLoadingImage: false,
  ...
  if ((gameState.isGameEnded || gameState.isGameFailed) && gameState.currentSegment) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 md:p-8 bg-gray-900 text-gray-100">
        <header className="w-full max-w-5xl mb-6 text-center">
          <h1 className={`font-press-start text-3xl tracking-tight ${isSuccess ? 'text-green-400' : 'text-red-500'}`}>
            {isSuccess ? (gameState.adventureOutline?.title || "Adventure Complete!") : "Your Journey Ends..."}
          </h1>
        ...
        <main className="w-full max-w-3xl bg-gray-800 p-8 rounded-lg shadow-xl text-center">
          <h2 className={`font-press-start text-2xl mb-6 ${isSuccess ? 'text-green-400' : 'text-red-400'}`}>
          ...
          {imageGenerationFeatureEnabled && !gameState.imageGenerationPermanentlyDisabled && gameState.currentSegment.imageUrl && (
          ...
          {gameState.imageGenerationPermanentlyDisabled && (
             <p className="text-sm text-yellow-400 italic my-4">Image generation was disabled due to usage limits.</p>
          )}
          ...
          {!isSuccess && (
             <p className="text-xl text-gray-400">Your quest for <span className="font-semibold text-purple-300">{gameState.adventureOutline?.overallGoal || "your goal"}</span> has come to an untimely end.</p>
          )}
  ...
  const isLoading = gameState.isLoadingOutline || gameState.isLoadingWorld || gameState.isLoadingStory || (gameState.isLoadingImage && imageGenerationFeatureEnabled && !gameState.imageGenerationPermanentlyDisabled) || isLoadingExamination;
  ...
  else if (gameState.isLoadingStory && (!dynamicLoadingText || (!storyLoadingTexts.includes(dynamicLoadingText) && !customActionLoadingTexts.includes(dynamicLoadingText)))) {
     dynamicLoadingText = gameState.lastRetryInfo?.customActionText && !gameState.lastRetryInfo.faultyJsonText ? getRandomLoadingText(customActionLoadingTexts) : getRandomLoadingText(storyLoadingTexts);
  }
  else if (gameState.isLoadingImage && imageGenerationFeatureEnabled && !gameState.imageGenerationPermanentlyDisabled && (!dynamicLoadingText || !imageLoadingTexts.includes(dynamicLoadingText))) dynamicLoadingText = getRandomLoadingText(imageLoadingTexts);
  ...
  const isImageOnlyError = gameState.error?.includes("Failed to load scene image:") && gameState.currentSegment;


  return (
    <div className="min-h-screen flex flex-col p-4 md:p-8 bg-gray-900 text-gray-100 items-center">
      {showImageQuotaNotification && (
        <div className="fixed top-0 left-0 right-0 bg-yellow-500 text-gray-900 p-3 text-center shadow-lg z-[100]" role="alert">
            <p>Image generation quota exceeded. Visuals have been disabled for this session to ensure gameplay continuity.</p>
            <button onClick={() => setShowImageQuotaNotification(false)} className="ml-4 px-2 py-1 bg-yellow-600 hover:bg-yellow-700 rounded text-sm font-semibold">&times; Dismiss</button>
        </div>
      )}
      <header className="w-full max-w-5xl mb-8 text-center">
        ...
        {gameState.adventureOutline && gameState.currentSegment && !gameState.isGameEnded && !gameState.isGameFailed && (
         ...
         <div className="mt-1 text-sm text-gray-500 space-y-0.5">
            ...
            {gameState.imageGenerationPermanentlyDisabled && <p className="text-yellow-400 italic">Image generation disabled (quota).</p>}
            {!imageGenerationFeatureEnabled && !gameState.imageGenerationPermanentlyDisabled && <p className="text-gray-600 italic">Image generation disabled (config).</p>}
        </div>
      </header>
      ...
      {gameState.currentSegment && !gameState.isGameEnded && !gameState.isGameFailed && (
